...
BEGIN
    ...
    SELECT COALESCE(MAX(CAST(SUBSTRING(Property_Code, 1, 4) AS SIGNED)), 0) INTO MAX_VAL
    FROM `dbo.property`;
    ...
    SET NEW.Property_Code = CONCAT(LPAD(MSHD_PREFIX_PARAM, 4, '0'), 'BDS', THANGHT_PARAM, NAMHT_PARAM);
